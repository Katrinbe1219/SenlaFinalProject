...
import org.example.application.dto.getting.goods.GoodForExportDto;
...
import org.example.application.hibernate.base_settings.exportEnum.*;
...
import org.example.application.services.documents.prices.PriceExportService;
import org.example.application.services.export.ExportCsvService;
import org.example.application.services.export.ExportXlsxService;
...
public class ExportController {
    ...
    // лист с отзывами: id, price, validFrom, validTo
    //csv only
    ...
    // таблица с продуктами: good_id, good_name - optional
    // таблица с категориями - optional list
    //таблица с тегами - optional list
    // лист с отзывами: id, rate, review, createdAt
    // csv and xslx
    public ResponseEntity<byte[]> getReviewsHistory(
            @RequestParam(value = "include", required = false) List<String> include,
            @RequestParam(value = "format", defaultValue = "csv") String format,
            @RequestBody ReviewAdvancedFilters filters
    ){
    ...
    // отдельная под продукты с id, good_name
