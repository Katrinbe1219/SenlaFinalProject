...
import org.example.application.exceptions.*;
import org.example.application.hibernate.base_settings.filters.UserForModeratorFilter;
import org.example.application.hibernate.documents.RefreshTokenHibImpl;
import org.example.application.hibernate.objects.UserHibImpl;
import org.example.application.mapping.users.UserForModeratorMapper;
import org.example.application.mapping.users.UserFullMapper;
...
import org.example.application.security.UserDetailsCustomService;
...
public class UserService {
    ...
    private UserForModeratorMapper mapper;
    private UserFullMapper fullMapper;
    ...
    @Transactional
    public List<UserForModeratorDto> getAllUsers(UserForModeratorFilter filters){
        List<User> users = userHib.getUsersForModerator(filters);
        if (users == null){
            return List.of();
        }
        return toList(users);
    ...
    public UserFullDto createUser(RoleTypes type, RegisterDto dto){
        try{
            ...
            return fullMapper.toDto(newUser);
        }
    ...
    private List<UserForModeratorDto> toList(List<User> users){
        List<UserForModeratorDto> dtos = new ArrayList<>();
        for (User user : users){
